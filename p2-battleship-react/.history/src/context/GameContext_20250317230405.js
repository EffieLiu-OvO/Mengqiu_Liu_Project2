...
import { placeShipsRandomly, canPlaceShip } from "../utils/shipPlacement";
...
const gameReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "START_PLACEMENT_PHASE":
      // 使用与 shipPlacement.js 中相同的船只类型配置
      const shipsToPlace = [
        { size: 5, id: "carrier" },
    ...
    case "PLACE_SHIP":
      ...
      const positions = [];
      const horizontal = state.shipOrientation === "horizontal";

      // 在棋盘上放置船只，使用与 placeShip 函数相同的逻辑
      for (let i = 0; i < shipToPlace.size; i++) {
        const shipRow = horizontal ? row : row + i;
        const shipCol = horizontal ? col + i : col;

        newPlayerBoard[shipRow][shipCol] = { hasShip: true, isHit: false };
        positions.push({ row: shipRow, col: shipCol });
      }

      // 创建新的船只对象，与 placeShipsRandomly 生成的对象结构一致
      const newShip = {
        positions,
      ...
      // 创建新的船只数组，添加新放置的船只
      const newPlayerShips = [...state.playerShips, newShip];
